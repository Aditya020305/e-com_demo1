  ...
  return (
    <section className="bg-neutral-900 min-h-[calc(100vh-4rem)]">
      <div className="max-w-7xl mx-auto px-4 md:px-8 py-8">
        ...
        <div className="space-y-4 mb-8">
          {cartItems.map((item) => (
            ...
            >
              ...
              {/* Quantity Controls */}
              <div className="flex items-center gap-0 rounded-xl border border-neutral-700 bg-neutral-900 overflow-hidden transition-all duration-200">
                <button
                  onClick={() => updateQuantity(item.id, item.quantity - 1)}
                  disabled={item.quantity <= 1}
                  className="px-3 py-2 text-neutral-400 hover:text-primary-400 hover:bg-neutral-700 transition-all duration-200 text-sm font-bold disabled:opacity-30 disabled:cursor-not-allowed"
                >
                  −
                </button>
                <span className="px-4 py-2 text-sm font-semibold text-neutral-100 min-w-[2.5rem] text-center border-x border-neutral-700">
                  {item.quantity}
                </span>
                <button
                  onClick={() => updateQuantity(item.id, item.quantity + 1)}
                  className="px-3 py-2 text-neutral-400 hover:text-primary-400 hover:bg-neutral-700 transition-all duration-200 text-sm font-bold"
                >
                  +
                </button>
              </div>
              ...
              {/* Line Total */}
              <p className="text-sm font-bold text-neutral-100 min-w-[5rem] text-right">
              ...
              <button
                ...
                className="p-2 rounded-lg text-neutral-600 hover:text-red-400 hover:bg-red-500/10 transition-all duration-200"
        ...
        {/* ── Total Section ── */}
        <div className="rounded-xl border border-neutral-800 bg-neutral-800/60 p-6 mb-6">
